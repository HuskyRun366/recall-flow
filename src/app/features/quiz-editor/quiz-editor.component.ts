...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterModule, ToonEditorComponent, GraphicalEditorComponent, ImportDialogComponent],
...
export class QuizEditorComponent implements OnInit {
  ...
  handleImportComplete(importedQuestions: ImportedQuestion[]): void {
    const quizId = this.quizId && this.quizId !== 'new' ? this.quizId : '';
    const existingQuestions = this.questions();

    // Convert imported questions to Question format
    const newQuestions: Question[] = importedQuestions.map((imported, index) => {
      const baseQuestion: Question = {
        id: '', // Will be generated by Firestore
        quizId: quizId,
        orderIndex: existingQuestions.length + index,
        type: 'multiple-choice',
        questionText: imported.questionText,
        createdAt: new Date(),
        updatedAt: new Date()
      };

      // For flashcard type, create a simple multiple choice with the answer
      if (imported.type === 'flashcard' || !imported.distractors || imported.distractors.length === 0) {
        baseQuestion.options = [
          {
            id: '0',
            text: imported.correctAnswer,
            isCorrect: true
          },
          {
            id: '1',
        ...
        const allOptions = [
          { text: imported.correctAnswer, isCorrect: true },
          ...imported.distractors.map(d => ({ text: d, isCorrect: false }))
        ];

        baseQuestion.options = allOptions.map((opt, idx) => ({
          id: idx.toString(),
          text: opt.text,
    ...
    this.questions.set([...existingQuestions, ...newQuestions]);
